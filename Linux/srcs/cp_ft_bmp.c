...
/*   Updated: 2020/12/22 21:19:53 by suchida          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	build_pixel_data(t_all *a, char *pixel_data, int x_pad)
{
	int	i;
	int	j;
	int	k;
	...
	while (i >= 0)
	{
		j = 0;
		while (j < a->e->win_x * 4)
			pixel_data[i * (a->e->win_x + x_pad) * 4 + j++] = a->img.addr[k++];
		p = 0;
		i--;
	}
}
...
void		make_bmp(t_all *a)
{
	...
	build_pixel_data(a, pixel_data, x_pad);
